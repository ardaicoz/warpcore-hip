...
#include <algorithm>
#include <random>
#include <warpcore/counting_hash_table.hpp>
#include <warpcore/bloom_filter.hpp>
...
// implement a custom HIP kernel that does the job.
...
int main()
{
    ...
    key_t* keys_in_h; hipHostMalloc(&keys_in_h, sizeof(key_t)*input_size);

    // allocate device-sided array for inputs
    key_t* keys_in_d; hipMalloc(&keys_in_d, sizeof(key_t)*input_size);
    ...
    // subsequently copy the input keys to the HIP device
    hipMemcpy(keys_in_d, keys_in_h, sizeof(key_t)*input_size, hipMemcpyHostToDevice);
    ...
    key_t*   keys_out_h;   hipHostMalloc(&keys_out_h,   sizeof(key_t)*input_size);
    count_t* counts_out_h; hipHostMalloc(&counts_out_h, sizeof(count_t)*input_size);
    uint64_t output_size_h;

    // allocate device-sided (pinned) arrays for output
    key_t*    keys_out_d;    hipMalloc(&keys_out_d,    sizeof(key_t)*input_size);
    count_t*  counts_out_d;  hipMalloc(&counts_out_d,  sizeof(count_t)*input_size);

    // we can retrieve all counts at once as follows
    hash_table.retrieve_all(keys_out_d, counts_out_d, output_size_h);

    // copy the results to the host
    hipMemcpy(keys_out_h, keys_out_d, sizeof(key_t)*output_size_h, hipMemcpyDeviceToHost);
    hipMemcpy(counts_out_h, counts_out_d, sizeof(count_t)*output_size_h, hipMemcpyDeviceToHost);
    ...
    hipFree(keys_in_d);
    hipHostFree(keys_out_h);
    hipFree(keys_out_d);
    hipHostFree(counts_out_h);
    hipFree(counts_out_d);
