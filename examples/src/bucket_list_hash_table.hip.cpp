...
#include <algorithm>
#include <random>
#include <warpcore/bucket_list_hash_table.hpp>
...
int main ()
{
    ...
    hipHostMalloc(&keys_unique_h, sizeof(key_t) * size_unique_keys); HIPERR
    key_t * keys_unique_d = nullptr;
    hipMalloc(&keys_unique_d, sizeof(key_t) * size_unique_keys); HIPERR

    key_t * keys_in_h = nullptr;
    hipHostMalloc(&keys_in_h, sizeof(key_t) * size); HIPERR
    key_t * keys_in_d = nullptr;
    hipMalloc(&keys_in_d, sizeof(key_t) * size); HIPERR

    value_t * values_in_h = nullptr;
    hipHostMalloc(&values_in_h, sizeof(value_t) * size); HIPERR
    value_t * values_in_d = nullptr;
    hipMalloc(&values_in_d, sizeof(value_t) * size); HIPERR

    index_t * offsets_out_h = nullptr;
    hipHostMalloc(&offsets_out_h, sizeof(index_t) * (size_unique_keys+1)); HIPERR
    index_t * offsets_out_d = nullptr;
    hipMalloc(&offsets_out_d, sizeof(index_t) * (size_unique_keys+1)); HIPERR

    value_t * values_out_h = nullptr;
    hipHostMalloc(&values_out_h, sizeof(value_t) * size); HIPERR
    value_t * values_out_d = nullptr;
    hipMalloc(&values_out_d, sizeof(value_t) * size); HIPERR

    status_t * status_h = nullptr;
    hipHostMalloc(&status_h, sizeof(status_t) * size); HIPERR
    status_t * status_d = nullptr;
    hipMalloc(&status_d, sizeof(status_t) * size); HIPERR
    ...
    hipMemcpy(keys_unique_d, keys_unique_h, sizeof(key_t)*size_unique_keys, H2D); HIPERR
    hipMemcpy(keys_in_d, keys_in_h, sizeof(key_t)*size, H2D); HIPERR
    hipMemcpy(values_in_d, values_in_h, sizeof(value_t)*size, H2D); HIPERR
    hipMemset(values_out_d, 0, sizeof(value_t)*size); HIPERR
    hipMemset(offsets_out_d, 0, sizeof(index_t)*(size_unique_keys+1)); HIPERR
    hipMemcpy(status_d, status_h, sizeof(status_t)*size, H2D); HIPERR
    ...
    hipMemcpy(status_h, status_d, sizeof(status_t)*size, D2H); HIPERR
