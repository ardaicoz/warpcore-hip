    ...
    std::chrono::milliseconds thermal_backoff = std::chrono::milliseconds(100))
{
    ...
    hipMalloc(&query_keys_d, sizeof(key_t)*max_keys); HIPERR
    value_t* values_d = nullptr;
    hipMalloc(&values_d, sizeof(value_t)*max_keys); HIPERR
    index_t * offsets_d = nullptr;
    hipMalloc(&offsets_d, sizeof(index_t)*(max_keys+1)); HIPERR

    hipMemset(values_d, 1, sizeof(value_t)*max_keys); HIPERR
    ...
    hipFree(query_keys_d); HIPERR
    hipFree(values_d); HIPERR
    hipFree(offsets_d); HIPERR
}
