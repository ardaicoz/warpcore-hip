#ifndef WARPCORE_BUCKET_LIST_HASH_TABLE_HPP
#define WARPCORE_BUCKET_LIST_HASH_TABLE_HPP

#include "single_value_hash_table.hpp"
...
public:
    ...
    */
     ...
     * \param stream HIP stream in which this operation is executed
     */
    HOSTQUALIFIER INLINEQUALIFIER
    void init(
        const key_type seed,
        const hipStream_t stream = 0) noexcept
    {
        ...
        const hipStream_t stream = 0) const noexcept
    {
        ...
        else
        {
            ...
            hipMemsetAsync(key_count, 0, sizeof(index_type), stream);
     ...
     */
    HOSTQUALIFIER INLINEQUALIFIER
    index_type bytes_keys(const hipStream_t stream = 0) const noexcept
    {
     ...
     */
    HOSTQUALIFIER INLINEQUALIFIER
    index_type bytes_values(const hipStream_t stream = 0) const noexcept
    {
     ...
     */
    HOSTQUALIFIER INLINEQUALIFIER
    index_type bytes_payload(const hipStream_t stream = 0) const noexcept
    {
    ...
    index_type num_values(const hipStream_t stream = 0) const noexcept
    {
        ...
        hipMemcpyAsync(&out, tmp, sizeof(index_type), D2H, stream);
...
#endif /* WARPCORE_BUCKET_LIST_HASH_TABLE_HPP */