#ifndef WARPCORE_STORAGE_HPP
#define WARPCORE_STORAGE_HPP
...
public:
        ...
        is_copy_(false)
    {
        if(capacity != 0)
        {
            ...
            {
                hipMalloc(&store_, sizeof(T) * capacity_);
    ...
    ~CyclicStore() noexcept
    {
        if(!is_copy_)
        {
            if(store_ != nullptr) hipFree(store_);
            delete current_;
        }
    }
    #endif
...
public:
        ...
        is_copy_(false)
    {
        if(capacity != 0)
        {
            ...
            {
                hipMalloc(&keys_, sizeof(key_type)*capacity);
                hipMalloc(&values_, sizeof(value_type)*capacity);
    ...
    ~SoAStore() noexcept
    {
        if(!is_copy_)
        {
            ...
            if(values_ != nullptr) hipFree(values_);
        }
    }
    #endif
     ...
     */
    HOSTQUALIFIER INLINEQUALIFIER
    void init_keys(const key_type key, const hipStream_t stream = 0) noexcept
    {
        if(!status_.has_any())
        {
     ...
     */
    HOSTQUALIFIER INLINEQUALIFIER
    void init_values(const value_type value, const hipStream_t stream = 0) noexcept
    {
        if(!status_.has_any())
        {
            helpers::lambda_kernel
...
public:
        ...
        is_copy_(false)
    {
        if(capacity != 0)
        {
            ...
            if(helpers::available_gpu_memory() >= total_bytes)
            {
                hipMalloc(&pairs_, sizeof(pair_t) * capacity);
    ...
    ~AoSStore() noexcept
    {
        if(!is_copy_)
        {
            if(pairs_ != nullptr) hipFree(pairs_);
        }
    }
    #endif
...
public:
        ...
        is_copy_(false)
    {
            ...
            max_bucket_size_ <= handle_type::max_bucket_size())
        {
            ...
            if(helpers::available_gpu_memory() >= total_bytes)
            {
                hipMalloc(&buckets_, sizeof(bucket_type) * capacity_);
                hipMalloc(&next_free_bucket_, sizeof(index_type));
    ...
    ~BucketListStore() noexcept
    {
        if(!is_copy_)
        {
            if(buckets_ != nullptr) hipFree(buckets_);
            if(next_free_bucket_ != nullptr) hipFree(next_free_bucket_);
        }
    }
    #endif
    ...
    {
        handle_type current_handle = handle;
     ...
     */
    HOSTQUALIFIER INLINEQUALIFIER
    index_type bytes_occupied(const hipStream_t stream = 0) const noexcept
    {
