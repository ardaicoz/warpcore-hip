#ifndef WARPCORE_MULTI_BUCKET_HASH_TABLE_HPP
#define WARPCORE_MULTI_BUCKET_HASH_TABLE_HPP
...
public:
    ...
    {
        hipMalloc(&status_, sizeof(status_type));
        hipMalloc(&num_keys_, sizeof(index_type));
        hipMalloc(&num_occupied_, sizeof(index_type));
    ...
    ~MultiBucketHashTable() noexcept
    {
        if(!is_copy_)
        {
            if(status_ != nullptr) hipFree(status_);
            if(num_keys_ != nullptr) hipFree(num_keys_);
            if(num_occupied_ != nullptr) hipFree(num_occupied_);
        }
    }
    #endif

    /*! \brief (re)initialize the hash table
     * \param[in] stream HIP stream in which this operation is executed in
     */
    HOSTQUALIFIER INLINEQUALIFIER
    void init(const hipStream_t stream = 0) noexcept
    {
        is_initialized_ = false;
            ...
            !temp_.status().has_not_initialized())
        {
            ...
            hipMemsetAsync(num_keys_, 0, sizeof(index_type), stream);
            hipMemsetAsync(num_occupied_, 0, sizeof(index_type), stream);
...
private:
        ...
        status_type& status) noexcept
    {
                       ...
                       i += cg_size())
        {
            ...
            auto empty_value_mask = detail::ballot_mask(group, is_empty_value(table_value));

            num_values += min(bucket_size(),cg_size()) - detail::popcount(empty_value_mask);
            ...
            while(empty_value_mask)
            {
                ...
                empty_value_mask = detail::clear_lane(empty_value_mask, leader);
            }
        }
...
public:
        ...
        const index_type probing_length = defaults::probing_length()) noexcept
    {
        ...
        for(index_type i = iter.begin(key_in, seed_); i != iter.end(); i = iter.next())
        {
            ...
            auto empty_key_mask = detail::ballot_mask(group, is_empty_key(table_key));

            const auto key_found_mask = detail::ballot_mask(group, table_key == key_in);

            if(key_found_mask)
            {
                last_key_pos = group.shfl(i, detail::last_set_bit(key_found_mask));
            }

            num_values_plus_bucket_size += bucket_size() * detail::popcount(key_found_mask);
            ...
            while(empty_key_mask)
            {
                ...
                empty_key_mask = detail::clear_lane(empty_key_mask, leader);
            }
        }
        ...
        typename StatusHandler::base_type * const status_out = nullptr) const noexcept
    {
        ...
        // hipcub::DeviceScan::InclusiveSum takes input sizes of type int
        ...
        if(values_out != nullptr)
        {
            ...
            hipMemsetAsync(begin_offsets_out, 0, sizeof(index_type), stream);
    ...
    void retrieve_all(
        ...
        const hipStream_t stream = 0) const noexcept
    {
        if(!is_initialized_) return;
    ...
    */
        ...
        const hipStream_t stream = 0) const noexcept
    {
        ...
        if(keys_out != nullptr)
        {
            ...
            hipMemsetAsync(tmp, 0, sizeof(index_type), stream);
            ...
            hipMemcpyAsync(&num_out, tmp, sizeof(index_type), D2H, stream);
     ...
     * \tparam Func type of map i.e. HIP device lambda
        ...
        const index_type probing_length = defaults::probing_length()) const noexcept
    {
        ...
        for(index_type i = iter.begin(key_in, seed_); i != iter.end(); i = iter.next())
        {
            ...
            const auto hit_mask = detail::ballot_mask(group, hit);

            index_type num_empty = 0;
            if(hit)
            {
                const auto j =
                    num + bucket_size() * detail::popcount(
                        hit_mask & detail::prefix_lane_mask(group.thread_rank()));
            ...
            if(hit_mask)
            {
                num_empty = group.shfl(num_empty, detail::last_set_bit(hit_mask));
            }

            num += bucket_size() * detail::popcount(hit_mask) - num_empty;
     ...
     */
    HOSTQUALIFIER INLINEQUALIFIER
    index_type num_keys(const hipStream_t stream = 0) const noexcept
    {
        index_type num = 0;

        hipMemcpyAsync(&num, num_keys_, sizeof(index_type), D2H, stream);
     ...
     */
    HOSTQUALIFIER INLINEQUALIFIER
    index_type num_occupied(const hipStream_t stream = 0) const noexcept
    {
        index_type num = 0;

        hipMemcpyAsync(&num, num_occupied_, sizeof(index_type), D2H, stream);
     ...
     */
    HOSTQUALIFIER INLINEQUALIFIER
    index_type num_values(const hipStream_t stream = 0) const noexcept
    {
        return size(stream);
    }
     ...
     */
    HOSTQUALIFIER INLINEQUALIFIER
    index_type size(const hipStream_t stream = 0) const noexcept
    {
        ...
        hipMemsetAsync(tmp, 0, sizeof(index_t), stream);
     ...
     */
    HOSTQUALIFIER INLINEQUALIFIER
    float key_load_factor(const hipStream_t stream = 0) const noexcept
    {
     ...
     */
    HOSTQUALIFIER INLINEQUALIFIER
    float value_load_factor(const hipStream_t stream = 0) const noexcept
    {
     ...
     */
    HOSTQUALIFIER INLINEQUALIFIER
    float storage_density(const hipStream_t stream = 0) const noexcept
    {
     ...
     */
    HOSTQUALIFIER INLINEQUALIFIER
    float relative_storage_density(const hipStream_t stream = 0) const noexcept
    {
        const index_type key_bytes = num_keys(stream) * sizeof(key_type);
        const index_type value_bytes = num_values(stream) * sizeof(value_type);
     ...
     */
    HOSTQUALIFIER INLINEQUALIFIER
    status_type peek_status(const hipStream_t stream = 0) const noexcept
    {
     ...
     */
    HOSTQUALIFIER INLINEQUALIFIER
    status_type pop_status(const hipStream_t stream = 0) noexcept
    {
        status_type status = status_type::not_initialized();
...
#endif /* WARPCORE_MULTI_BUCKET_HASH_TABLE_HPP */
