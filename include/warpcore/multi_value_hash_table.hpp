#ifndef WARPCORE_MULTI_VALUE_HASH_TABLE_HPP
#define WARPCORE_MULTI_VALUE_HASH_TABLE_HPP
...
public:
        ...
        const index_type probing_length = defaults::probing_length()) noexcept
    {
        ...
        for(index_type i = iter.begin(key_in, seed_); i != iter.end(); i = iter.next())
        {
            ...
            auto empty_mask = detail::ballot_mask(group, is_empty_key(table_key));

            num_values += detail::popcount(detail::ballot_mask(group, table_key == key_in));
            ...
            while(empty_mask)
            {
                ...
                empty_mask = detail::clear_lane(empty_mask, leader);
            }
        }
        ...
        typename StatusHandler::base_type * const status_out = nullptr) const noexcept
    {
        ...
        if(values_out != nullptr)
        {
            ...
            else
            {
                ...
                hipError_t err = hipMalloc(&cubtemp, required_temp_bytes);
    ...
    */
        ...
        const index_type probing_length = defaults::probing_length()) const noexcept
    {
        ...
        for(index_type i = iter.begin(key_in, seed_); i != iter.end(); i = iter.next())
        {
            ...
            {
                const auto j =
                    num + detail::popcount(hit_mask & detail::prefix_lane_mask(group.thread_rank()));

                f(key_in, table_[i].value, j);
            }

            num += detail::popcount(hit_mask);
     ...
     */
    HOSTQUALIFIER INLINEQUALIFIER
    float load_factor(const hipStream_t stream = 0) const noexcept
    {
...
#endif /* WARPCORE_MULTI_VALUE_HASH_TABLE_HPP */
