#ifndef WARPCORE_HASH_SET_HPP
#define WARPCORE_HASH_SET_HPP
...
public:
        ...
        is_copy_(false)
    {
        ...
        {
            hipMalloc(&keys_, sizeof(key_type) * capacity_);
            hipMalloc(&status_, sizeof(Status));
    ...
    ~HashSet() noexcept
    {
        if(!is_copy_)
        {
            if(keys_   != nullptr) hipFree(keys_);
    ...
    void retrieve_all(
        key_type * keys_out,
        index_type& num_out,
        hipStream_t stream = 0) const noexcept
    {
        if(!is_initialized_) return;
        ...
        index_type probing_length = defaults::probing_length()) noexcept
    {
        ...
        for(index_type i = iter.begin(key_in, seed_); i != iter.end(); i = iter.next())
        {
            ...
            auto hit_mask = detail::ballot_mask(group, hit);
     ...
     */
    HOSTQUALIFIER INLINEQUALIFIER
    index_type size(hipStream_t stream = 0) const noexcept
    {
     ...
     */
    HOSTQUALIFIER INLINEQUALIFIER
    float load_factor(hipStream_t stream = 0) const noexcept
    {
     ...
     */
    HOSTQUALIFIER INLINEQUALIFIER
    float storage_density(hipStream_t stream = 0) const noexcept
    {
     ...
     */
    HOSTQUALIFIER INLINEQUALIFIER
    Status peek_status(hipStream_t stream = 0) const noexcept
    {
     ...
     */
    HOSTQUALIFIER INLINEQUALIFIER
    Status pop_status(hipStream_t stream = 0) noexcept
    {
        Status status = Status::not_initialized();
